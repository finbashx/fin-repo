...
const FeatchartY = (ccoins) => {
  ...
  const getRecent = () => {
    let tempr = [...new Array(365)].map((i, idx) =>moment().startOf("day").subtract(idx, "days").format('D/MM'));
  ...
  useEffect(() => {
    {
      if(ccoins.ccoins  && localStorage.getItem("STOP_RENDER1") == "false"){
        let tempchart=[];
      for (let i = 0; i < (ccoins.ccoins[0]).length; i++){
        try{
      fetch(`https://min-api.cryptocompare.com/data/v2/histoday?fsym=${ccoins.ccoins[0][i].coin}&tsym=GBP&limit=${ 365}&api_key=45ff14f28775ec177bc0a52c15c5efb9695792a3fe7c3dcfafeef756350e6d1c`)
        .then(resp => resp.json())
        .then(resp => {
          Object.values(resp);
      ...
      if(datasetss.length>=ccoins.ccoins[0].length){
        setChart1(datasetss);
        getRecent();
        localStorage.setItem("STOP_RENDER1","true")
      }
    }
    }
  });
